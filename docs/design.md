...
review against the ticket requirements, stores the run in SQLite, and writes a
Markdown report.
...
The prompt is built from `src/prompts/review-prompt.md` by substituting
`{{TICKET}}`, `{{REPO}}`, `{{BRANCH}}`, `{{BASE_SHA}}`, `{{CHANGED_FILES}}`, and
`{{PREVIOUS_FINDINGS}}` (open blocker and major findings from the previous run of
the same repo and branch, so a re-review can confirm fixes).

The prompt instructs the model to restate the ticket as a numbered requirement
list, read the diff and the surrounding code, honour the reviewed repository's
own `CLAUDE.md` / `AGENTS.md` conventions, check each requirement with evidence,
report only real defects, never edit anything, and end with exactly one fenced
`json` block matching `ReviewOutput`. `can_merge` is true only when every
requirement is `met` and there is no blocker or major finding.

`stream-json` output is parsed line by line; assistant text and tool names are
forwarded to the log so the UI can show progress. The final `type: "result"`
message carries the answer. The JSON payload is extracted from the last fenced
`json` block, falling back to the last balanced `{...}`, and validated with zod.
On validation failure the runner retries once with a short corrective prompt; a
second failure throws with the raw text attached. `REVIEW_TIMEOUT_MS` bounds the
run, and the process tree is killed on timeout or cancel.
...
(`met|partial|missing|not_verifiable`) mirror the columns above. `TicketInfo`,
`ResolvedTarget`, and `CheckoutResult` are the values passed between the
resolver, the sandbox, and the runner. `ReviewOutput` is the exact JSON shape the
review model must return: `summary`, `requirements[]`, `findings[]`, `verdict`,
`can_merge`, `conclusion`.

## Queue and events

`src/queue.ts` runs reviews sequentially, concurrency 1, in-process. On server
...
| GET | `/api/reviews/:id` | | `{ review, requirements, findings, logs }` |
...
not_verifiable ❓), findings grouped by severity with `file:line`, and a
